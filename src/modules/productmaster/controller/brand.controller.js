...
//         if (!data.brand_name) return res.status(400).json({ message: "brand_name is required" });

//         const newRecord = await brandService.create(data);
//         return res.status(201).json({ message: "Brand created successfully", data: newRecord });
//     } catch (error) {
...
//     try {
//         const { id } = req.params;
//         const [updatedRowsCount, updatedRows] = await brandService.update(id, req.body);
